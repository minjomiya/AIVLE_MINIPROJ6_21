...
function App() {
  ...
  useEffect(() => {
    async function loadBooks() {
      try {
        const res = await fetch("http://3.144.149.46:8080/books");
  ...
  const handleUpdateBook = async (updatedBook) => {
    try {
    const res = await fetch(`http://3.144.149.46:8080/books/${updatedBook.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleGenerateImage = async (selectedBook, selectedQuality) => {
    ...
    try {
      ...
      const updateRes = await fetch(
        `http://3.144.149.46:8080/books/${selectedBook.id}/cover`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
